...
layout_num = [[sg.Text('Ввдеите количество исследуемых объектов в выборке:')],      #Окто ввода количества исследуемых объектов
              [sg.Input(key='num', size=(5, 10)), sg.Text('Не более 15 объектов')],
              [sg.Button('Ввести'), sg.Exit('Выход')]
              ]

layout_main = [[sg.Text('Введите координаты исследуемых объектов')],                #Окто ввода координат исследуемых объектов
...
while True:                             #Петля первого окна
    ...
    else:                                           #Обработка ввдеденного количества исследуемых объектов
...
while True: #Петля второго окна
    ...
    if event == 'Начать исследование':      #запуск главного исследования
        def main():
            ...
            if '' in tech:
                ...
                while i < len(values) / 2 - 1:          #обработка входящих параметров объектов
                ...
                print()
                first_module = forel.Forel(points, radius)      #Работа модуля форель
                ...
                print()
                second_module = s_pic.Picture(first_module[0], first_module[1], radius)     #Работа модуля изображения
                ...
                print('____________________________')
                print()
                print('Расчет завершен успешно. Данные визуализированы на изображении.\n'
                      'Координаты центров масс кластеров и входящих в кластера точки\n'
                      'приведены в расчетах первого модуля.')
        main()
